// Save layer0 plaintext (the 469-byte E1 blob) to disk for PowerShell analysis.
// Usage: node extract-e1.cjs <userDataDir> <secretFile> <outFile>
...
async function main() {
  const child = spawn(EXE, [`--user-data-dir=${ud}`, `--inspect=${PORT}`], {
    env: { ...process.env, V2_DISABLE_AUTO_UPDATE: '1', T2R22_TARGET: target, T2R22_OUT: outFile },
    stdio: 'ignore',
  });
  let list = null;
  for (let i = 0; i < 40; i++) {
  ...
  const expr = `(() => {
    ...
    try {
      ...
      fs.writeFileSync(process.env.T2R22_OUT, b);
      return JSON.stringify({ ok: true, len: b.length, head: b.subarray(0, 4).toString() });
    } catch (e) { return JSON.stringify({ ok: false, err: String(e) }); }
  })()`;
  const r = await send('Runtime.evaluate', { expression: expr, returnByValue: true });
  console.log('EXTRACT:', r.result?.result?.value ?? JSON.stringify(r, null, 2));
  sock.close();
