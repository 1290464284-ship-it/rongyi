...
- 随访提醒新增完成接口与前端操作，支持 PENDING/IN_PROGRESS 到 COMPLETED，缺失返回 404，非法状态返回冲突，并支持记录可选完成结果。
- 随访页按已逾期、今日待随访、后续待随访分组展示，并保留完成操作。
...
- 新增采购单明细查询接口，并在库存采购操作页展示采购单条目，便于收货前后核对数量、单价和金额。
...
- FTS5 搜索已完成，不再属于后续建议。
- Web 核心文件覆盖率已超过 98% statements / 100% lines，不再属于后续建议。
- ReactBits 实验目录、jsrepo 配置和实验依赖已全部删除，隔离配置已清理。
- BOSS 多诊所经营视图已新增 `GET /api/v2/analytics/clinic-overview`。
- 批量导入已支持事务分块，默认 100 行/块，单请求上限提升到 10000 行。
- 随访已支持批量完成和 CSV 导出。
- 安装升级 smoke 已加入“备份创建、校验、恢复 staging 后再升级”的组合验证。
