...
| T3.6 | 上传配额 + dev seed 显式开关（M5/M6-sec） | Wave 2 | seed 门覆盖 R2-P2-16：非 production 下默认**不**重置 REDACTED，仅当 `V2_ALLOW_DEV_SEED=1` 时重置/播种 demo |
...
按承接清单顺序执行 T3.2 → T3.3（在 T2R-01 基础上收紧，保留 file:// 与 'null' 放行）→ T3.4（含 backup-key 静默重生成改显式确认）→ T3.5（含公式注入转义 + phone 入 SENSITIVE_FIELDS）→ T3.6（含 dev seed 门：非 production 默认不重置 REDACTED，`V2_ALLOW_DEV_SEED=1` 才重置）→ T3.7（含 updater 签名校验、token 内存回退、崩溃上报最小化）。
