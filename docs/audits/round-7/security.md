...
- 审计对象：`D:/Desktop/rongyi/apps/v2`（Electron + Vite + React + Express + better-sqlite3，382 个 TS/TSX/CJS/MJS 文件，约 7 万行）
- 审计日期：2026-08-07
...
### M4. 非生产默认凭据：NODE_ENV 缺省即 development，首启种 admin/REDACTED 与 doctor/REDACTED；V2_ALLOW_DEV_SEED=1 每次启动重置 admin 密码

- **位置**：`src/server/main.ts:193`（`NODE_ENV ?? 'development'`）、`src/server/infrastructure/database.ts:354-397`（seedDatabase：非生产种 admin（默认 `REDACTED`，可用 `V2_ADMIN_PASSWORD` 覆盖）+ doctor/REDACTED + demo 数据；`NODE_ENV==='development' && V2_ALLOW_DEV_SEED==='1'` 时**每次启动重置 admin 密码为 seedPassword**）
- **问题描述**：
  1. 生产部署若忘记设置 `NODE_ENV=production`（手动 `npm start` / 直接跑 server.cjs），主进程按 development 处理：JWT secret 每次重启随机（main.ts:203）、种子账户被创建——首次启动即存在 `admin/REDACTED` 与 `doctor/REDACTED` 两个可登录账户。
...
- **攻击场景**：运维用默认方式启动（未设 NODE_ENV）并配 `V2_HOST=0.0.0.0` 后，攻击者用 `admin/REDACTED` 直接登录获得 BOSS 全权限。
- **修复建议**：
...
5. **M4** `src/server/infrastructure/database.ts:354-397` + `src/server/main.ts:193`：非生产默认凭据（admin/REDACTED、doctor/REDACTED）与每次启动重置开关
