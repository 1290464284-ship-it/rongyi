...
### H6 — 仓库根 `apps/v2` 与本地 `source/apps/v2` 双份工作副本漂移（嵌套 git 仓库）

- 位置：`D:/Desktop/rongyi/apps/v2` vs `D:/Desktop/rongyi/source/apps/v2`（diff 实证：`installer-smoke.ps1`、`verify-signature.ps1`、`legacy/schema/*.ts`、`src/server/http/routes/system.ts`、多个 web 页面及 spec 文件不一致；`source` 是嵌套 git 仓库且 remote 与根相同）
...
### I4 — 默认管理员密码 `REDACTED` 出现在 README、api-smoke、installer/upgrade smoke
