...
interface RequestWithCookies extends Request { cookies: Record<string, string>; }
...
describe('AuthController', () => {
  ...
  describe('login', () => {
    it('登录成功设置 cookies 并返回用户信息', async () => {
      const dto: LoginDto = { username: 'admin', password: '0801' };
      const loginResult = {
    ...
    it('登录需要修改密码时 needChangePassword 为 true', async () => {
      const dto: LoginDto = { username: 'user', password: '1234' };
  ...
  describe('refresh', () => {
    it('使用 cookie 中的 refresh_token 刷新', async () => {
      const req = { cookies: { refresh_token: 'cookie-token' } };
    ...
    it('cookie 不存在时直接抛出 BusinessValidationException（不再支持 body 参数）', async () => {
      const req = { cookies: {} };
    ...
    it('未提供 refreshToken 时抛出 BusinessValidationException', async () => {
      const req = { cookies: {} };
  ...
  describe('logout', () => {
    it('登出并清除 cookies', async () => {
      const user: JwtUser = { id: 'u-1', username: 'admin', name: '管理员', role: 'BOSS' };
      auth.logout.mockResolvedValue({ success: true });

      const result = await controller.logout(user, res as unknown as Response);
  ...
  describe('updateUser', () => {
    it('更新基本字段（name, role, phone）', async () => {
      const dto: UpdateUserDto = { name: '更新名', role: 'NURSE', phone: '13900139000' };
    ...
    it('active 字段为 true 时转换为 1', async () => {
      const dto: UpdateUserDto = { active: true };
    ...
    it('active 字段为 false 时转换为 0', async () => {
      const dto: UpdateUserDto = { active: false };
    ...
    it('未传 active 字段时 updates 中不包含 active', async () => {
      const dto: UpdateUserDto = { name: '只更新名字' };
