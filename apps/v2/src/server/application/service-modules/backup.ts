...
export class BackupService {
  ...
  async mirrorBackup(
    filename: string,
    mirrorDir: string,
  ): Promise<{ filename: string; target: string; fileSize: number; sha256: string }> {
    const source = this.safePath(filename);
    if (!fs.existsSync(source)) throw new NotFoundError('Backup file not found');
    fs.mkdirSync(mirrorDir, { recursive: true });
    const target = path.join(mirrorDir, path.basename(filename));
    ...
    try {
      ...
      return { filename, target, fileSize: fs.statSync(target).size, sha256 };
    } catch (error) {
      try {
  ...
  mirrorCleanup(mirrorDir: string, keep = 30): { kept: number; deleted: Array<string> } {
    ...
    for (const name of files.slice(0, Math.max(0, files.length - keepCount))) {
      try {
        ...
        this.logger?.warn('[backup] failed to delete mirror backup during cleanup', {
          filename: name,
          error: error instanceof Error ? error.message : error,
        });
      }
