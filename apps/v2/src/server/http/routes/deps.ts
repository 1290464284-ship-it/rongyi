...
export function createRouteDependencies(input: RouteDependenciesInput): RouteDependencies {
  ...
  return {
    ...
    rateLimitStore,
    stocktakeLockGuard,
    authService: new AuthService(db, undefined, logger),
    ...
    backups: new BackupService(db, dbPath, backupDir, logger),
    stats: new StatsService(db),
    sync: new SyncService(db),
    hr: new HrService(db),
    alerts: new AlertService(db),
    memberCards: new MemberCardService(db),
    purchaseOrders: new PurchaseOrderService(db, undefined, undefined, stocktakeLockGuard),
