...
describe('HTTP app', () => {
  ...
  beforeAll(async () => {
    ...
    const login = await request(app).post('/api/v2/auth/login').send({ username: 'admin', password: 'ry0801' });
  ...
  it('handles null-clinic uploads and missing file records on disk', async () => {
    ...
    const nullLogin = await request(app)
      ...
      .send({ username: 'file-null-clinic', password: 'ry0801' })
      .expect(200);
    await request(app)
      .post('/api/v2/files')
  ...
  it('supports auth profile, password change, and resource CRUD', async () => {
    ...
    await request(app).patch('/api/v2/auth/password')
      .set('Authorization', `Bearer ${token}`)
      .send({ oldPassword: 'ry0801', newPassword: 'newpass123' })
      .expect(200);
