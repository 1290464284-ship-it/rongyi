...
describe('purchase-orders/form', () => {
  ...
  it('builds valid items with inventory names or a custom fallback', () => {
    const rows = [
    ...
    const valid = buildValidItems(rows as never, [{ id: 'i-1', name: '耗材' }] as never);
    expect(valid).toHaveLength(2);
    expect(valid[0]).toMatchObject({ itemId: 'i-1', name: '耗材', quantity: 2, unitPrice: 10000 });
