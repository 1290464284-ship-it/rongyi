...
const drill = createDrill({
  appRoot,
  serverScript,
  legacyDb: dirtyLegacy,
  legacySchemaDir,
  dataDir,
  backupDir,
  logDir,
  port,
  jwtSecret: 'legacy-dirty-jwt-0123456789abcdef0123456789abcdef',
  backupKey: 'legacy-dirty-backup-key-0123456789abcdef',
  adminPassword,
  readyLabel: 'legacy dirty drill',
});

const { spawnApi, stopApi } = drill;
...
function waitForApiOrExit(timeoutMs = 45_000) {
  ...
  return new Promise((resolve) => {
    ...
    drill.apiProcess.once('exit', (code) => finish({ ok: false, code }));
    const attempt = async () => {
